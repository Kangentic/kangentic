...
4. **Gather the diff (union; each command its own Bash call).** Capture all three layers - they are disjoint, so concatenating never double-counts:
   ...
   If the committed diff, the uncommitted diff, **and** the untracked list are all empty, emit "No changes to review." and stop. Otherwise `diffText` = committed diff + uncommitted diff + the synthetic untracked blocks. Also compute the compact **signature delta** from the diff alone (the integration finder consumes it; see "## Finders"). `changedFiles` comes from the build script's `paths:` line once it has run (below) - never from `--stat` output, which truncates long paths.
   ...
   **Build the shared review pack (gather once, read N times).** Run `node scripts/build-review-pack.mjs <base>` (one Bash call; omit `<base>` when the base is empty). The script assembles `.kangentic/REVIEW_PACK.tmp.md` - `Total lines: <N>` header, a table of contents with exact start lines, the full union diff (untracked files as synthetic added-file blocks), then line-numbered bodies of the changed files, largest churn first, capped near 200KB with trimmed files listed under `## Not included (read on demand)` - AND it writes `.kangentic/REVIEW_PREEXISTING_DIRTY.tmp` from the same gather, printing only a short summary. Verify both files exist after the run. A body whose changed hunks cover only part of it is packed as `## Partial file:` - every changed hunk with 20 lines of context, with the unchanged runs between them replaced by a marked, line-numbered gap - instead of `## Full file:`. Which files get a body is decided on full-body cost exactly as before, so windowing never displaces a file the pack would otherwise carry; it only makes the pack smaller (measured across eight merged PRs: identical coverage, 2% to 71% fewer pack bytes, and every pack byte is re-read by up to 11 finders). If the script is missing (older checkout), fall back to building both files by hand per the layouts above and the dirty-list rules below. This exists because the 2026-08-29 audit (`docs/code-review-fanout-audit.md`) measured each finder independently re-gathering the diff (50-78k tokens each) and re-reading the same changed files (38% of all Read bytes duplicated): the pack pays that cost once. Two rules protect the savings: the pack is delivered as a FILE PATH, never embedded in finder prompts and never written through the `Write` tool (both re-bill the pack as driver output tokens - ~100k tokens at Opus rates for a 200KB pack - which is why the script does the writing); and `.kangentic/` is gitignored, so neither tmp file ever stages or needs cleanup.
   ...
   **Take `changedFiles` from the script's `paths:` line.** The summary block prints two distinct lines, and only the second carries paths:
   ...
   `changedFiles` = that comma-separated list, verbatim. It is the script's own `changedFiles` array - the deduped union of all three layers, in full untruncated form - so it needs no reconstruction and cannot disagree with what the pack was built from. Read the `paths:` line, never the `changed files:` count line above it (counts, not paths), and never the pack's `## Contents` TOC: the TOC lists only files whose BODY was packed, so a file trimmed by the 200KB cap (or binary/oversized) is absent from it, and every path dropped from `changedFiles` silently un-gates a domain auditor whose glob it matched (a trimmed `src/main/db/migrations.ts` skips `migration-safety`; a trimmed `src/main/pty/**` skips `platform-guard`) while the review still reports success. The `paths:` line lists packed and trimmed files alike, which is exactly why it is the source.
