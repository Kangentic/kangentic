...
| `task:spawnBlocked` | on | Event: the task was created, promoted or unarchived, but its agent could not start because another task holds the same checkout. Those paths deliberately keep the task, so without this the result is indistinguishable from a healthy spawn |
