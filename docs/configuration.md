...
| `hasCompletedFirstRun` | boolean | `false` | Legacy: set true on first task creation, kept for schema/fixture compatibility. No onboarding UI reads it; see `onboardedProjectIds`. Auto-set, not shown in UI. |
...
| `onboardedProjectIds` | string[] \| undefined | `undefined` | Project ids whose onboarding checklist the user has dismissed. `undefined` means the one-time upgrade backfill (on first app hydration) has not run yet; `[]` means it has run and nothing is dismissed. Global, keyed by project id like `lastActiveTaskByProject`. Auto-set, not shown in UI. |
| `onboardingBaseline` | Record\<string, object\> | `{}` | Per-project snapshot of the settings the onboarding checklist watches (`defaultAgent`, `defaultModel`, `defaultEffort`, `permissionMode`, and a `swimlaneSignature` string encoding of the board's shape), captured when the project is added or on first checklist open. Checklist steps 1 and 2 tick when live state DIFFERS from this, so opening a settings screen and closing it unchanged earns no checkmark. Keyed by project id; replaced wholesale on write (a `CONFIG_DICTIONARY_PATHS` entry). Auto-set, not shown in UI. |
