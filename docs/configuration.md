...
| `monitor.stateFilter` | MonitorStateBucket[] | `[]` | Attention buckets to show. Empty means every bucket. Like `projectFilter`, not written by any control today and cleared on load - `liveOnly` covers the case users actually asked for. |
