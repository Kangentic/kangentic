...
src/main/mobile-bridge/       # desktop implementation, consumes @kangentic/protocol
  ...
  handlers/                   # one module per capability verb, registered once at attachContext()
    ...
    board-tool-allowlist.ts      # per-tool read/mutate classification (excludes query_db + create_column/delete_column + move_task/list_tasks/list_columns/list_backlog; browser/devtools/diagnostics/cross-project tools are absent from commandHandlers, so they are excluded for free)
...
- `isSecureRelayAddress` now parses the URL authority instead of prefix-matching it. `ws://127.0.0.1:8080@evil.test` previously returned true, because everything before an `@` in an authority is credentials: that address dials `evil.test`, and since the pairing token IS the Noise PSK, dialed verbatim as the relay's `?slot=`, one crafted QR field put the PSK on the wire in cleartext to an attacker-chosen host, which `activePairing` then persisted as the trust anchor for every later session. The loopback carve-out sits outside any dev gate, so this reached production builds.
