  ...
  server.registerTool(
    'kangentic_get_transcript',
    {
      ...
      inputSchema: z.object({
        ...
        aroundUuid: z.string().optional().describe('Structured only. Center the returned entries on the turn with this uuid (the turnUuid from a kangentic_recall / kangentic_search_everything conversation hit). Returns that turn plus `context` turns either side - the precise citation-first fetch. Ignored for raw.'),
