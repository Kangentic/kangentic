...
export function registerTaskTools(
  ...
  server.registerTool(
    'kangentic_get_current_task',
    {
      description: 'Resolve the Kangentic task that corresponds to the current working directory and/or git branch. Use this at the start of work in a worktree to confirm which task you are operating on (e.g. before commits, PRs, or merge-back). Pass the agent\'s CWD and/or current branch name. Matches against tasks.worktree_path (full path or .kangentic/worktrees/<folder> segment) and tasks.branch_name. Returns the same shape as kangentic_find_task, plus `devPort`: the dev-server port reserved for THIS task. Start any dev server on that port (e.g. `npm start -- --port <devPort>`) instead of the framework default - several tasks share one board column and run at the same time, so a default port collides with a sibling task and leaves you verifying against ITS app instead of your own. Null means no port is reserved; use your own default then.',
      inputSchema: z.object({
