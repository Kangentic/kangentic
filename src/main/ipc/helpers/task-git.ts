 ...
 * column override -> project default -> global fallback), because skipping
 * the worktree commits us to remote execution: if this function and the
 * spawn-time resolution disagree, a LOCAL agent spawns with
 * `cwd = projectPath` (transition-engine's `task.worktree_path || projectPath`
 * fallback) and runs unisolated in the user's main checkout.
 *
