...
import { SyncDefaultsModal } from './SyncDefaultsModal';
...
export function AppSettingsPanel() {
  ...
  const skipDefaultsSyncConfirm = useConfigStore((state) => state.skipDefaultsSyncConfirm);
  ...
  const handleDefaultUpdate = (partial: DeepPartial<AppConfig>) => {
    if (skipDefaultsSyncConfirm) {
      // Auto-save, no sync to existing projects
      updateConfig(partial);
    } else {
      // Save immediately, then ask about syncing
      updateConfig(partial);
      setPendingSyncPartial(partial);
    }
  };

  const handleSyncConfirm = (syncToProjects: boolean, dontAskAgain: boolean) => {
    if (dontAskAgain) {
      useConfigStore.getState().setSkipDefaultsSyncConfirm(true);
    }
    if (syncToProjects && pendingSyncPartial) {
  ...
  return (
    <SettingsPanelShell title="App Settings" onClose={handleClose}>
      {/* ── Appearance ── */}
      <SectionHeader label="Appearance" />
      <SettingRow label="Theme" description="Color scheme for the interface">
        <Select
          value={globalConfig.theme}
          onChange={(event) => handleAppUpdate({ theme: event.target.value as ThemeMode })}
        >
      ...
      <SectionHeader
        ...
        description="Default values for new projects. Changing these can optionally update all existing projects."
        prominent
      />
