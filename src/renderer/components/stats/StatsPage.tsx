...
import { ChartColumn, Info, X } from 'lucide-react';
 ...
 * through the usage-push debounce + poll below, animating in place.
 */
export function StatsPage() {
  ...
  // React.memo holds across unrelated StatsPage re-renders.
  ...
  return (
    ...
    >
      ...
      >
        {/* Header: page identity top-left (the analytics convention - it also
            avoids colliding with the app title bar's centered project name),
            then ONE left-anchored toolbar whose order encodes precedence:
            WHAT (scope), LENS (dollars/tokens), then WHEN (range pills +
            custom window + drill chip) behind a hairline divider. */}
        <div className="flex items-center gap-2.5 px-4 pt-3 pb-2 flex-shrink-0">
          <ChartColumn size={18} className="text-fg-muted" aria-hidden />
