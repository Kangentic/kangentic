...
describe('withGuest - capability gating', () => {
  ...
  it('blocks all capabilities when automation is disabled', async () => {
    const result = await withGuest({ selector: {}, capability: 'observe', config: config({ enabled: false }) }, async () => 'ran');
  ...
  it('blocks interaction when allowInteraction is off', async () => {
    const result = await withGuest({ selector: {}, capability: 'interact', config: config({ allowInteraction: false }) }, async () => 'ran');
  ...
  it('blocks navigation when allowNavigation is off', async () => {
    const result = await withGuest({ selector: {}, capability: 'navigate', config: config({ allowNavigation: false }) }, async () => 'ran');
    expect(result).toMatchObject({ ok: false, error: { kind: 'navigation-disabled' } });
  });

  it('blocks eval when allowEval is off', async () => {
    const result = await withGuest({ selector: {}, capability: 'eval', config: config() }, async () => 'ran');
    expect(result).toMatchObject({ ok: false, error: { kind: 'eval-disabled' } });
  });

  it('allows observe even when interaction/eval are off', async () => {
    const result = await withGuest({ selector: {}, capability: 'observe', config: config() }, async () => 'snapshot');
...
describe('withGuest - resolution and attach', () => {
  ...
  it('attaches lazily then runs the body', async () => {
    browserPaneRegistry.register({ sessionId: 's', taskId: 't', projectId: 'p', webContentsId: 7, url: null });
    seedGuest(7);
    const result = await withGuest({ selector: { sessionId: 's' }, capability: 'observe', config: config() }, async (wc) => (wc as { id: number }).id);
  ...
  it('does not re-attach when the debugger is already attached', async () => {
    ...
    await withGuest({ selector: { sessionId: 's' }, capability: 'observe', config: config() }, async () => 'ok');
  ...
  it('reports pane-destroyed when the guest no longer resolves', async () => {
    ...
    const result = await withGuest({ selector: { sessionId: 's' }, capability: 'observe', config: config() }, async () => 'ok');
    expect(result).toMatchObject({ ok: false, error: { kind: 'pane-destroyed' } });
  });

  it('reports no-pane-open when nothing is registered', async () => {
    const result = await withGuest({ selector: { sessionId: 'missing' }, capability: 'observe', config: config() }, async () => 'ok');
  ...
  it('wraps a thrown body error as driver-error', async () => {
    browserPaneRegistry.register({ sessionId: 's', taskId: 't', projectId: 'p', webContentsId: 7, url: null });
    seedGuest(7);
    const result = await withGuest({ selector: { sessionId: 's' }, capability: 'observe', config: config() }, async () => {
