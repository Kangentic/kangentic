...
import { slugify, computeSlugBudget } from '../../src/shared/slugify';
...
describe('Slugify Logic', () => {
  ...
  it('truncates long titles to keep folder names short', () => {
    const slug = slugify('update preview to copy local settings json for azure deploy');
    expect(slug.length).toBeLessThanOrEqual(20);
    // Folder name = slug + '-' + 8-char shortId = max 29 chars
    const folderName = `${slug}-abc12345`;
    expect(folderName.length).toBeLessThanOrEqual(29);
  });

  it('custom branch names are truncated for folder names', () => {
    const customBranch = 'feature/very-long-descriptive-branch-name-for-azure-deploy';
    const slug = slugify(customBranch.replace(/\//g, '-'), 20);
    const folderName = `${slug || 'task'}-abc12345`;
    expect(folderName.length).toBeLessThanOrEqual(29);
  });
});

describe('computeSlugBudget', () => {
  it('returns max 20 for short project paths', () => {
    expect(computeSlugBudget('C:\\dev\\myapp')).toBe(20);
  });

  it('returns reduced budget for long project paths', () => {
    const longPath = 'C:\\Users\\dev\\' + 'deep-nested-dir\\'.repeat(8) + 'project';
    const budget = computeSlugBudget(longPath);
    expect(budget).toBeGreaterThanOrEqual(0);
    expect(budget).toBeLessThan(20);
  });

  it('returns 0 for extremely deep project paths', () => {
    const veryLongPath = 'C:\\' + 'x'.repeat(200);
    expect(computeSlugBudget(veryLongPath)).toBe(0);
  });

  it('accounts for worktree prefix and build output reserve', () => {
    // With a 100-char project path:
    // 260 - 100 - 21 - 9 - 80 = 50, capped at 20
    const path100 = 'C:\\' + 'x'.repeat(97);
    expect(path100.length).toBe(100);
    expect(computeSlugBudget(path100)).toBe(20);

    // With a 160-char project path:
    // 260 - 160 - 21 - 9 - 80 = -10, floored at 0
    const path160 = 'C:\\' + 'x'.repeat(157);
    expect(path160.length).toBe(160);
    expect(computeSlugBudget(path160)).toBe(0);
  });
});
